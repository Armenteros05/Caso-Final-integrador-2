...
public:
    ...
    [[maybe_unused]] std::string to_json_string();

    Variant from_json_string(const std::string& sjson);
};
...
[[maybe_unused]] std::string Variant::to_json_string() {
    switch (type) {
        case variant_type::Symbol:
            return "\"" + val + "\"";
        case variant_type::Number:
...
Variant Variant::from_json_string(const std::string& sjson) {
    //debería poner algo pero no me da tiempo :(
}
